 function MainHPccUpOffDiag(par)
...
for n = 1:n_HM_models
    ...
    for m = 1:n_HM_cycles
        ...
        p = diag(0.999*ones(1, max_HMM_states(n))); % transition matrix (p) initialized
        updiag =diag((max_HMM_states(n)-sum(sum(p)))/(max_HMM_states(n)-1)*ones(max_HMM_states(n)-1,1),1);
        if isempty(updiag)
            p=p;
        else
        p =  p + updiag;
