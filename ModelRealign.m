function star = ModelRealign(star)
%% function 'star = ModelRealign'
% Reads-in parameters for each HM model type, then computes the
% transitioned realigned putative peri-stimulus histogram that gave rise to
% the model
...
for i = 1:star.n_datasets
    ...
    for j = 1:star.n_tastants
        ...
        beststate = squeeze(state_sequence(i, j, star.best_index(i)));
        data_to_use = squeeze(probability(beststate, :, :));
        data_to_use = (data_to_use > star.threshold) .* data_to_use;
        star.probability_thresholded{i, j} = data_to_use;
        star.beststate(i, j) = beststate;
        
        for n = 1:n_trials(i, j)
            
            [vals indices] = find(data_to_use(:, n)');
            if isempty(indices)
                indices = (n_bins(i, j) - 1) * ones(1, n_bins(i, j));
            end
        ...
        star.state_onset_times{i, j} = state_onset_times;
        ...
        for n = 1:n_trials(i, j)
            ...
            for m = 1:index
                ...
                realigned_trial_start = n_bins(i, j) - state_onset_times(n);
                temp(m, realigned_trial_start:realigned_trial_start + n_bins(i, j) - 1, n) = probability(m, :, n);
                realigned_probability(m, :, n) = temp(m, n_bins(i, j) - star.startpoint_realignment + (1:n_bins(i, j)) - 1, n);
        ...
        clear emission_probability_product emission_probability_innerproduct emission probability index realigned_probability
...
for i = 1:star.n_datasets
    ...
    switch star.plot_indiv_datasets_toggle
        case('on')
            ...
            addtitle('ModelRealign', 24, [0.5 0.1]);
